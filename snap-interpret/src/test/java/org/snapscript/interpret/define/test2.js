	...
	function timeTests(){
		for(var f in this.class.functions){
			if(f.name.startsWith("test")){
				...
				out.println("Time for "+f.name+"() was "+duration+" ms");
			}
		}
	}
}
...
class SpecialSpecial extends Special{
	...
	foo(x){
		out.println("SpecialSpecial.foo");
	}
	test(){
		...
		out.println("x="+format.format(x)+" super.x="+format.format(super.x)+" super.z="+format.format(super.z));
	}
}
class StaticMethod{
	static max(a,b){
...
class SimpleTest extends TestTimer{
	function testCount(){
		...
		out.println("result of 1 million iterations is="+format.format(j));
	}
