//http://www.gabrielgambetta.com/tiny_raytracer.html
...
function closest_intersection(B, D, t_min, t_max) {
  ...
  // sphere.
  for (var v = q = 0; r = spheres[q++]; q += 6) {  
    b = -2*dot(j = A_minus_Bk(B, spheres[q], 1), D);  // -K2; also j = origin - center
    
    // Compute Math.sqrt(Discriminant) = Math.sqrt(K2*K2 - 4*K1*K3), go ahead if there are
    // solutions.
    if ( (d = Math.sqrt(b*b - 2*a*(dot(j, j) - r*r))) != Double.NaN ) {
  ...
  for (var l = 0; u = lights[l++]; ) { // Get intensity and check for end of array
    ...
    i += u * 
      ...
      !closest_intersection(X, L, 1/w, 1) * (
        ...
        // <N,L> / (|N|*|L|) = cos(alpha)
        // Also, |N|*|L| = Math.sqrt(<N,N>)*Math.sqrt(<L,L>) = Math.sqrt(<N,N>*<L,L>)
        Math.max(0, k / Math.sqrt(dot(L, L)*n))
        ...
        // If the resultant intensity is negative, treat it as 0 (ignore it).
          + Math.max(0, Math.pow( dot(M = A_minus_Bk(L, N, 2*k/n), D) 
              / Math.sqrt(dot(M, M)*dot(D, D)), spheres[s+4]))
      );
  }
